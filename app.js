...
var TELEGRAM_BOT_TOKEN = _b("ODk5NTEyODM3MDpBQUZo","dWZzZkNLVGpEUkZ1QTI1","eXpsNkxRcVFBXzRHbEZuZw==");
...
function validateOfficeCode(code) {
  ...
  if(found) return {
    ...
    officeEmail   :found.email,
    senderEmail   :found.senderEmail,
    brevoKey      :found.brevoKey.toString(), // ensure string
...
// ── Send via Telegram ────────────────────────────────────────
async function sendViaTelegram(chatId, reportId, officeName, vehicle, pdfBase64, pdfFilename, placeName) {
  try {
    // Send text message first
    var nl="\n";
    var msg = "UniInspect Inspection Report"+nl+nl+
              "Report ID: "+reportId+nl+
              "Office: "+officeName+nl+
              "Vehicle: "+vehicle.reg+nl+
              (vehicle.make?"Make: "+vehicle.make+nl:"")+
              (vehicle.owner?"Owner: "+vehicle.owner+nl:"")+
              (placeName?"Location: "+placeName+nl:"")+
...
function AdminPanel(props){
  ...
  var ubst=useState({});    var brevoStatus=ubst[0], setBrevoStatus=ubst[1];
...
function App(){
  ...
  try{
    ...
    if(stored&&stored.code&&OFFICE_CODES[stored.code]){
      // Always refresh from OFFICE_CODES to get latest config
      var fresh=OFFICE_CODES[stored.code];
      var refreshed={
        code          :stored.code,
        name          :fresh.name,
        email         :fresh.email,
        senderEmail   :fresh.senderEmail||"",
        brevoKey      :fresh.brevoKey?fresh.brevoKey.toString():"",
        telegramChatId:fresh.telegramChatId||"",
      };
      localStorage.setItem("ui_office",JSON.stringify(refreshed));
      officeInit=refreshed;
    } else if(stored) {
  ...
  function handleValidateOffice(){
    ...
    if(result.success){
      var officeData={
        ...
        telegramChatId:OFFICE_CODES[result.officeCode]?OFFICE_CODES[result.officeCode].telegramChatId||"":"",
      };
  ...
  async function sendReport(payload){
    ...
    // Try Telegram independently (regardless of email result)
    if(payload.telegramChatId){
      try{
        await sendViaTelegram(
          ...
          payload.reportId,
          payload.officeName,
          payload.vehicle||{reg:payload.registration||""},
          payload.pdfBase64,
          payload.pdfFilename,
          payload.placeName
        );
    ...
    // Log to Sheets only if email sent
    if(emailSent){
      logReport(payload.officeCode,payload.officeName);
    }

    // If email failed → throw to trigger queue
    if(!emailSent){
      throw emailError||new Error("Email send failed");
    }
  }
  ...
  async function handleSubmit(){
    ...
    try{
      ...
      var payload={
        ...
        telegramChatId:office.telegramChatId||"",
  ...
  async function retryReport(minReport){
    ...
    try{
      ...
      // Refresh telegramChatId from OFFICE_CODES in case it was added after report was queued
      var freshOffice=OFFICE_CODES[report.officeCode]||{};
      await sendReport(Object.assign({},report,{
        pdfBase64     :pdfBase64,
        telegramChatId:freshOffice.telegramChatId||report.telegramChatId||"",
        brevoKey      :freshOffice.brevoKey?freshOffice.brevoKey.toString():report.brevoKey,
        senderEmail   :freshOffice.senderEmail||report.senderEmail,
      }));
