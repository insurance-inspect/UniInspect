...
var OFFICE_CODES = {
  ...
  "300501": { name:"UIIC BO KALPETTA",        email:"uiicinspection@gmail.com"   },
};
  ...
  function onTouchMove(e){
    ...
    if(e.touches.length===2){
      ...
      if(lastDistRef.current){
        var delta=dist/lastDistRef.current;
      ...
      if(lastPosRef.current){
        var dx=e.touches[0].clientX-lastPosRef.current.x;
        var dy=e.touches[0].clientY-lastPosRef.current.y;
...
function App(){
  ...
  useEffect(function(){
    var interval=setInterval(async function(){
      ...
      if(expired.length>0){ await refreshQueue(); return; }

      // Retry today's reports - try one at a time
      ...
      if(!fullReport){
        await removeFromQueue(minimalReport.reportId);
        await refreshQueue();
        return;
      }
      try {
        ...
        await refreshQueue();
        console.log("Auto sent report:", fullReport.reportId);
      } catch(e){
        console.log("Auto retry failed:", e.message);
      }
    },30000);
  ...
  return React.createElement("div",{style:S.page},
    ...
    // Full screen photo preview with pinch zoom
...
var S={
  ...
  previewImgWrap:{flex:1,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",position:"relative"},
  previewImg    :{maxWidth:"100%",maxHeight:"100%",objectFit:"contain",transformOrigin:"center center",transition:"transform 0.05s",userSelect:"none",WebkitUserSelect:"none"},
