...
const CLIENT_ID     = _d("Mjk0NzU1NTkwMzY2LWw2Y2NmdjJtNmVi","cGEub3RwbzF2MnV2ZXZpNmNhZ2Y0ZDhk","cHMuZ29vZ2xldXNlcmNvbnRlbnQuY29t");
const CLIENT_SECRET = _d("R09DU1BYLTFPd0o=","UWpqY3VnOEJ2ekRB","Y2tWNmdOQy00aVVj");
const REFRESH_TOKEN = _d("MS8vMDQ2VFItd0kzekdETENnWUlBUkFBR0FRU053Ri1MOQ==","Vi1JSXNEVkpobHhyOW05TWs2VnUzTUd5SzhXY2ptcERySQ==","TlBELTVQNjB4OERwaUR6NHFTR2lNamo0b0lMTXNoMFAwa28=");
const SENDER_EMAIL  = _d("bm9yZXBseS51bmk=","ZG5lcy50Y2Vwc25p","ZXJAZ21haWwuY29t");
const SENDER_NAME   = "UniInspect";
const QUEUE_KEY     = "ui_pending_queue";
const DB_NAME       = "UniInspectDB";
const DB_VERSION    = 1;
...
var OFFICE_CODES = {
  ...
  "300401": { name:"UIIC BO BADAGARA",        email:"300401.vadakara@gmail.com"  },
};
...
// Get verified internet time - tries multiple sources
async function getTrueTime() {
  ...
  // Source 3: Use server response Date header as fallback
  try {
    var resp3 = await fetch("https://www.google.com",{cache:"no-store",mode:"no-cors"});
    // If request goes through, internet is available - use adjusted phone time
    // Phone time is acceptable if internet confirmed available
...
// Check if date is still today (IST)
function isSameDay(dateStr) {
  try {
    var saved = new Date(dateStr);
    var now   = new Date();
    return saved.getDate()===now.getDate() &&
...
// Queue management — uses IndexedDB for unlimited storage
function getQueue() {
  try { return JSON.parse(localStorage.getItem(QUEUE_KEY)||"[]"); }
  catch(e) { return []; }
}

async function addToQueue(report) {
  await dbAdd(report);
  // Also keep minimal info in localStorage for quick count
  try {
    var q = JSON.parse(localStorage.getItem(QUEUE_KEY)||"[]");
    if(!q.find(function(r){return r.reportId===report.reportId;})) {
      q.push({reportId:report.reportId,registration:report.registration,
              officeName:report.officeName,savedAt:report.savedAt,placeName:report.placeName});
...
// Restamp unverified photos with sent time before sending delayed report
async function restampDelayedPhotos(photos, sentTime) {
  var restamped = Object.assign({}, photos);
  for(var slotId in photos) {
    var ph = photos[slotId];
    if(ph && !ph.verified && ph.originalDataUrl) {
      var slot = PHOTO_SLOTS.find(function(s){return s.id===slotId;});
      if(slot) {
        restamped[slotId] = Object.assign({}, ph, {
          url: await stampImage(
            ph.originalDataUrl,
            slot.label,
            formatTs(ph.ts),
            ph.gps,
...
function stampImage(dataUrl,label,timestamp,gps,placeName,verified,sentTime) {
  return new Promise(function(res) {
    var img=new Image();
    img.onload=function(){
      var fh   = Math.max(13, Math.round(img.width * 0.030));
      var pad  = Math.round(fh * 0.6);
      var locationLine = placeName ? placeName : (gps ? (gps.lat+", "+gps.lng) : "GPS: Unavailable");

      // Build lines based on internet time availability
      var lines  = [];
      var colors = [];
      ...
      if(verified && timestamp){
        // Internet time available at capture
        lines.push("🕐 "+timestamp+" IST ✓");
        colors.push("#fbbf24");
      } else if(sentTime){
        // No internet at capture — show sent time
        lines.push("🌐 "+sentTime+" IST");
        colors.push("#60a5fa");
        lines.push("(Offline at capture — sent time)");
        colors.push("#94a3b8");
      }
      // else no time shown at all if no internet and not yet sent

      lines.push(locationLine);
      colors.push("#86efac");

      // Measure strip height
      var ctx0 = document.createElement("canvas").getContext("2d");
      ctx0.font = "bold "+fh+"px monospace";
      var maxW = 0;
      lines.forEach(function(l){ var w=ctx0.measureText(l).width; if(w>maxW) maxW=w; });

      var stripH = fh * lines.length + pad * (lines.length + 1);

      // Create new canvas: original image + bottom strip
      var c   = document.createElement("canvas");
      c.width = img.width;
      c.height= img.height + stripH;

      var ctx = c.getContext("2d");

      // Draw original photo (clean — no overlay on photo)
      ctx.drawImage(img, 0, 0);

      // Draw black strip below photo
      ctx.fillStyle = "#0f172a";
      ctx.fillRect(0, img.height, c.width, stripH);

      // Draw text in strip
      ctx.font = "bold "+fh+"px monospace";
      lines.forEach(function(line, i){
        ctx.fillStyle = colors[i] || "#ffffff";
        ctx.fillText(line, pad, img.height + pad*(i+1) + fh*(i+1));
      });
...
// Calculate compression based on total photos captured
function getCompressionSettings(totalPhotos) {
  if(totalPhotos<=5) return {maxW:1080, quality:0.72};
  if(totalPhotos===6) return {maxW:1000, quality:0.68};
  if(totalPhotos===7) return {maxW:900,  quality:0.64};
  ...
  return new Promise(function(resolve) {
    ...
    img.onload = function() {
      try {
        ...
        // Threshold tuned for phone cameras
        // Phone cameras auto-focus well so threshold needs to be higher
        resolve({ blurred: variance < 200, variance: Math.round(variance) });
      } catch(e) {
        resolve({ blurred: false, variance: 999 });
      }
    };
    img.onerror = function() { resolve({ blurred: false, variance: 999 }); };
    img.src = dataUrl;
  });
}
...
async function buildPDF(opts) {
  ...
  var ry2=ly+24;
  // Remarks section
  if(opts.remarks&&opts.remarks.trim()){
    ...
    doc.text("INSPECTOR REMARKS",margin+6,ry2+7);
    doc.setFontSize(8);doc.setFont("helvetica","normal");doc.setTextColor(15,23,42);
    var rmLines=doc.splitTextToSize(opts.remarks,W-margin*2-12);
    doc.text(rmLines,margin+6,ry2+14);
    ry2=ry2+20+4;
  }
  ...
  for(var i=0;i<capturedSlots.length;i++){
    ...
    await new Promise(function(resolve){
      ...
      tmp.onload=function(){
        ...
        // Blur warning badge on PDF
        if(ph.blurry){
          doc.setFillColor(245,158,11);doc.roundedRect(px+2,py+2,38,8,2,2,"F");
          doc.setFontSize(6);doc.setFont("helvetica","bold");doc.setTextColor(255,255,255);
          doc.text("⚠️ LOW CLARITY",px+4,py+7);
        }
        var sy=py+ph_h+4;
        ...
        doc.setFontSize(9);doc.setFont("helvetica","bold");doc.setTextColor(15,23,42);
        doc.text(formatTs(ph.ts)||"Time unavailable",px+5,sy+14);
        doc.setFontSize(7);doc.setFont("helvetica","bold");doc.setTextColor(21,128,61);
...
function App(){
  ...
  var ublr=useState(null);             var blurWarning=ublr[0],   setBlurWarning=ublr[1];
  var ublrd=useRef({});                // store blurry slot data
  ...
  // Auto retry every 30 seconds — expire reports at end of day
  useEffect(function(){
    var interval=setInterval(async function(){
      var q=refreshQueue();
      if(q.length===0) return;

      // Check and expire reports from previous days
      var expired=q.filter(function(r){return !isSameDay(r.savedAt);});
      for(var ei=0;ei<expired.length;ei++){
        await removeFromQueue(expired[ei].reportId);
      }
      if(expired.length>0) await refreshQueue();

      // Retry today's reports
      var todayQ=getQueue().filter(function(r){return isSameDay(r.savedAt);});
      if(todayQ.length===0) return;
      var minimalReport=todayQ[0];
      // Get full report from IndexedDB
      var fullReport=await getFullReport(minimalReport.reportId);
      if(!fullReport) { await removeFromQueue(minimalReport.reportId); await refreshQueue(); return; }
      try {
        await trySendReport(fullReport);
  ...
  async function captureFromCamera(){
    ...
    // Show photo INSTANTLY in square box
    setPhotos(function(p){
      var n=Object.assign({},p);
      n[slotId]={url:dataUrl,ts:null,gps:null,placeName:null,processing:true,verified:false};
      return n;
    });
    ...
    setPhotos(function(p){
      var n=Object.assign({},p);
      n[slotId]={url:stamped,ts:ts,gps:photoGps,placeName:photoPlace,processing:false,verified:tsVerified,originalDataUrl:resized};
      return n;
    });
  ...
  // Process photo after blur check passed
  async function processPhoto(slotId, dataUrl) {
    var slot = PHOTO_SLOTS.find(function(s){return s.id===slotId;});
    var totalCaptured = Object.keys(photos).filter(function(k){return photos[k]&&!photos[k].processing;}).length + 1;
    var compress = getCompressionSettings(totalCaptured);
    var resized  = await resizeImage(dataUrl, compress.maxW, compress.quality);
  ...
  async function retryReport(minReport){
    ...
    try {
      ...
      if(hasUnverified && report.photos) {
        // Restamp photos with sent time
        var restampedPhotos = await restampDelayedPhotos(report.photos, sentTime);
        // Rebuild PDF with restamped photos
        var doc = await buildPDF({
          vehicle    : report.vehicle,
          photos     : restampedPhotos,
          generatedAt: formatTs(new Date(report.savedAt)),
        ...
        // Also save updated PDF to device
        doc.save(report.pdfFilename.replace(".pdf","_sent.pdf"));
      }

      await sendViaGmail(report.to, report.subject, report.bodyText, pdfBase64, report.pdfFilename);
  ...
  async function retryAll(){
    ...
    for(var i=0;i<q.length;i++){
      try{
        var full=await getFullReport(q[i].reportId);
        if(full){
          await trySendReport(full);
          await removeFromQueue(q[i].reportId);
        }
      }catch(e){}
    }
    await refreshQueue();
          ...
          isSameDay(report.savedAt)?"⏳ Expires today at midnight":"❌ Expired — cannot send"
                ...
                React.createElement("div",{style:{display:"flex",gap:8}},
                  React.createElement("button",{
                    ...
                    onClick:function(){if(!isSameDay(report.savedAt)){alert("This report has expired. It cannot be sent as the inspection date has passed.");return;}retryReport(report);},
  ...
  return React.createElement("div",{style:S.page},
    ...
    // Blur warning modal
    blurWarning&&React.createElement("div",{style:S.blurOverlay},
      React.createElement("div",{style:S.blurModal},
        React.createElement("div",{style:{fontSize:48,marginBottom:10}},"⚠️"),
        React.createElement("h3",{style:{fontSize:18,fontWeight:800,color:"#0f172a",marginBottom:8}},"Photo May Be Blurry"),
        React.createElement("p",{style:{fontSize:13,color:"#64748b",marginBottom:20,lineHeight:1.5}},
          "The captured photo appears to be blurry. A clear photo is important for insurance inspection."
        ),
        React.createElement("div",{style:{display:"flex",gap:10,justifyContent:"center"}},
          React.createElement("button",{
            style:{background:"#0f172a",color:"#fff",border:"none",borderRadius:10,padding:"12px 24px",fontSize:14,fontWeight:700,cursor:"pointer"},
            onClick:function(){
              var slotId=blurWarning;
              setBlurWarning(null);
              openCamera(slotId);
            }
          },"📷 Retake Photo"),
          React.createElement("button",{
            style:{background:"#f1f5f9",color:"#64748b",border:"1px solid #e2e8f0",borderRadius:10,padding:"12px 24px",fontSize:14,fontWeight:700,cursor:"pointer"},
            onClick:async function(){
              var slotId=blurWarning;
              var dataUrl=ublrd.current[slotId];
              setBlurWarning(null);
              // Show processing
              setPhotos(function(p){var n=Object.assign({},p);n[slotId]={url:dataUrl,ts:null,gps:null,placeName:null,processing:true,verified:false,blurry:true};return n;});
              await processPhoto(slotId, dataUrl);
              // Mark as blurry
              setPhotos(function(p){var n=Object.assign({},p);if(n[slotId])n[slotId].blurry=true;return n;});
            }
          },"Keep Anyway")
        )
      )
              ...
              cap
                ?React.createElement(React.Fragment,null,
                    ...
                    !cap.processing&&React.createElement("div",{style:S.photoBar},
                      ...
                      cap.blurry&&React.createElement("span",{style:{fontSize:9,color:"#f59e0b",fontWeight:700}},"⚠️ BLUR"),
...
var S={
  ...
  blurOverlay   :{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:250,display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"},
  blurModal     :{background:"#fff",borderRadius:20,padding:"30px 24px",maxWidth:340,width:"100%",textAlign:"center",boxShadow:"0 20px 60px rgba(0,0,0,0.3)"},
