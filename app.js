...
// Email: Brevo (primary) / Gmail (toggle)

const { useState, useRef, useCallback, useEffect } = React;

const BACKEND_URL = "https://script.google.com/macros/s/AKfycbzDq0HroG07DZZVW8mtasWmkSxehHHwAuMGaqP5vXLAstEeI_vU14hvAVsk4XVEC9EuHA/exec";
...
async function getTrueTime() {
  try {
    var resp=await fetch("https://worldtimeapi.org/api/timezone/Asia/Kolkata",{cache:"no-store"});
    var data=await resp.json();
    if(data&&data.datetime) return {date:new Date(data.datetime),verified:true};
  } catch(e){}
  try {
    var resp2=await fetch("https://timeapi.io/api/time/current/zone?timeZone=Asia%2FKolkata",{cache:"no-store"});
    var data2=await resp2.json();
    if(data2&&data2.dateTime) return {date:new Date(data2.dateTime),verified:true};
  } catch(e){}
  try {
...
async function backendGet(action, params) {
  try {
    var url=BACKEND_URL+"?action="+action;
...
async function backendPost(payload) {
  try {
    var resp=await fetch(BACKEND_URL,{method:"POST",body:JSON.stringify(payload)});
    return await resp.json();
  } catch(e){return {error:e.message};}
}

// ── Email Senders ────────────────────────────────────────────
async function sendViaBrevo(apiKey, to, subject, bodyText, pdfBase64, pdfFilename) {
  var payload={
    sender     :{name:"UniInspect",email:"noreply.uniinspect.sender@gmail.com"},
    to         :[{email:to}],
    subject    :subject,
...
function _d(a,b,c){var t=atob;return t(a)+t(b).split("").reverse().join("")+t(c);}

async function getGmailAccessToken(clientId,secret,token) {
  var resp=await fetch("https://oauth2.googleapis.com/token",{
    method:"POST",
    headers:{"Content-Type":"application/x-www-form-urlencoded"},
    body:new URLSearchParams({client_id:clientId,client_secret:secret,refresh_token:token,grant_type:"refresh_token"})
  });
  var data=await resp.json();
...
async function sendViaGmail(clientId,secret,refreshToken,to,subject,bodyText,pdfBase64,pdfFilename) {
  var accessToken=await getGmailAccessToken(clientId,secret,refreshToken);
  var boundary="uninspect"+Date.now();
  var nl="\r\n";
  var email=[
    "MIME-Version: 1.0",
    "To: "+to,
    "From: UniInspect <noreply.uniinspect.sender@gmail.com>",
    "Reply-To: noreply.uniinspect.sender@gmail.com",
    "Subject: "+subject,
    "X-Mailer: UniInspect",
    "List-Unsubscribe: <mailto:noreply.uniinspect.sender@gmail.com>",
  ...
  var raw=btoa(unescape(encodeURIComponent(email))).split("+").join("-").split("/").join("_").split("=").join("");
...
function stampImage(dataUrl,label,timestamp,gps,placeName,verified,sentTime){
  return new Promise(function(res){
    ...
    img.onload=function(){
      ...
      c.width=img.width;c.height=img.height+stripH;
...
function AdminPanel(props){
  ...
  var utab=useState("settings"); var tab=utab[0],  setTab=utab[1];
  var uset=useState(null); var settings=uset[0],   setSettings=uset[1];
  var uoff=useState([]);   var offices=uoff[0],    setOffices=uoff[1];
  var ubk=useState([]);    var brevoKeys=ubk[0],   setBrevoKeys=ubk[1];
  var ust=useState({});    var stats=ust[0],        setStats=ust[1];
  var uload=useState(false); var loading=uload[0], setLoading=uload[1];
  var unoff=useState({code:"",name:"",email:""}); var newOffice=unoff[0],setNewOffice=unoff[1];
  var unbk=useState({officeCode:"",apiKey:"",label:""}); var newBrevo=unbk[0],setNewBrevo=unbk[1];

  async function login(){
    setLoading(true);
    var result=await backendGet("getSettings");
    setLoading(false);
    if(result.AdminPassword===adminPass){
      setAuthed(true);setSettings(result);
      loadAll();
    } else {
  ...
  async function loadAll(){
    setLoading(true);
    var [off,bk,st]=await Promise.all([
      backendGet("getOffices"),
      backendGet("getBrevoKeys"),
      backendGet("getStats"),
    ]);
    setOffices(Array.isArray(off)?off:[]);
    setBrevoKeys(Array.isArray(bk)?bk:[]);
    setStats(st&&!st.error?st:{});
    setLoading(false);
  }

  async function toggleSender(sender){
    await backendPost({action:"updateSetting",password:adminPass,key:"EmailSender",value:sender});
    setSettings(function(s){return Object.assign({},s,{EmailSender:sender});});
  }

  async function saveOffice(){
    if(!newOffice.code||!newOffice.name||!newOffice.email){alert("Fill all fields");return;}
    await backendPost({action:"addOffice",code:newOffice.code,name:newOffice.name,email:newOffice.email});
    setNewOffice({code:"",name:"",email:""});
    loadAll();
  }

  async function deleteOffice(code){
    if(!window.confirm("Remove office "+code+"?")) return;
    await backendPost({action:"removeOffice",code:code});
    loadAll();
  }

  async function saveBrevoKey(){
    if(!newBrevo.officeCode||!newBrevo.apiKey){alert("Fill office code and API key");return;}
    await backendPost({action:"addBrevoKey",officeCode:newBrevo.officeCode,apiKey:newBrevo.apiKey,label:newBrevo.label});
    setNewBrevo({officeCode:"",apiKey:"",label:""});
    loadAll();
  }

  async function deleteBrevoKey(code){
    if(!window.confirm("Remove Brevo key for "+code+"?")) return;
    await backendPost({action:"removeBrevoKey",officeCode:code});
    loadAll();
  }
  ...
  if(!authed) return React.createElement("div",{style:S.adminOverlay},
    React.createElement("div",{style:S.adminModal},
      ...
      React.createElement("div",{style:{display:"flex",gap:10}},
        React.createElement("button",{style:Object.assign({},S.btn,{flex:1}),onClick:login,disabled:loading},loading?"Checking...":"Login"),
  ...
  return React.createElement("div",{style:S.adminOverlay},
    React.createElement("div",{style:Object.assign({},S.adminModal,{maxWidth:480,maxHeight:"90vh",overflowY:"auto",padding:0})},
      // Header
      ...
      // Tabs
      React.createElement("div",{style:{display:"flex",borderBottom:"1px solid #e2e8f0",background:"#f8fafc"}},
        [["settings","⚙️ Settings"],["offices","🏢 Offices"],["brevo","🔑 Brevo"],["stats","📊 Stats"]].map(function(t){
          return React.createElement("button",{key:t[0],
            style:{flex:1,padding:"10px 4px",border:"none",borderBottom:tab===t[0]?"2px solid #1d4ed8":"2px solid transparent",
              background:"transparent",fontSize:11,fontWeight:tab===t[0]?700:400,color:tab===t[0]?"#1d4ed8":"#64748b",cursor:"pointer"},
      ...
      React.createElement("div",{style:{padding:"16px"}},
        ...
        // SETTINGS TAB
        tab==="settings"&&settings&&React.createElement("div",null,
          React.createElement("div",{style:{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:12}},"Email Sender"),
          React.createElement("div",{style:{display:"flex",gap:10,marginBottom:16}},
            React.createElement("button",{
              style:{flex:1,padding:"12px",borderRadius:10,border:"2px solid "+(settings.EmailSender==="brevo"?"#1d4ed8":"#e2e8f0"),
                background:settings.EmailSender==="brevo"?"#eff6ff":"#f8fafc",
                color:settings.EmailSender==="brevo"?"#1d4ed8":"#64748b",fontWeight:700,cursor:"pointer",fontSize:13},
              onClick:function(){toggleSender("brevo");}
            },"📧 Brevo"+(settings.EmailSender==="brevo"?" ✓":"")),
            React.createElement("button",{
              style:{flex:1,padding:"12px",borderRadius:10,border:"2px solid "+(settings.EmailSender==="gmail"?"#1d4ed8":"#e2e8f0"),
                background:settings.EmailSender==="gmail"?"#eff6ff":"#f8fafc",
                color:settings.EmailSender==="gmail"?"#1d4ed8":"#64748b",fontWeight:700,cursor:"pointer",fontSize:13},
              onClick:function(){toggleSender("gmail");}
            },"📬 Gmail"+(settings.EmailSender==="gmail"?" ✓":""))
          ),
          React.createElement("div",{style:{background:"#f0fdf4",borderRadius:10,padding:"12px",marginBottom:16}},
            React.createElement("div",{style:{fontSize:12,fontWeight:700,color:"#15803d",marginBottom:4}},"Current Sender"),
            React.createElement("div",{style:{fontSize:14,fontWeight:800,color:"#0f172a"}},
              settings.EmailSender==="brevo"?"📧 Brevo API":"📬 Gmail API"
            )
          ),
          React.createElement("div",{style:{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:8}},"Gmail Credentials"),
          ["GmailClientId","GmailSecret","GmailToken"].map(function(key){
            return React.createElement("div",{key:key,style:{marginBottom:8}},
              React.createElement("label",{style:{fontSize:11,color:"#64748b",display:"block",marginBottom:3}},key),
              React.createElement("div",{style:{display:"flex",gap:6}},
                ...
                React.createElement("button",{
                  style:{background:"#0f172a",color:"#fff",border:"none",borderRadius:8,padding:"0 12px",fontSize:11,cursor:"pointer"},
                  onClick:async function(){
                    var val=document.getElementById("admin_"+key).value;
                    await backendPost({action:"updateSetting",password:adminPass,key:key,value:val});
                    alert("Saved!");
                  }
                },"Save")
              )
        ...
        // OFFICES TAB
        tab==="offices"&&React.createElement("div",null,
          React.createElement("div",{style:{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:10}},"Add New Office"),
          React.createElement("input",{style:Object.assign({},S.input,{marginBottom:8,fontSize:12}),placeholder:"Office Code (e.g. 300601)",value:newOffice.code,onChange:function(e){setNewOffice(function(o){return Object.assign({},o,{code:e.target.value.toUpperCase()});})}}),
          React.createElement("input",{style:Object.assign({},S.input,{marginBottom:8,fontSize:12}),placeholder:"Office Name",value:newOffice.name,onChange:function(e){setNewOffice(function(o){return Object.assign({},o,{name:e.target.value});})}}),
          React.createElement("input",{style:Object.assign({},S.input,{marginBottom:8,fontSize:12}),placeholder:"Recipient Email",value:newOffice.email,onChange:function(e){setNewOffice(function(o){return Object.assign({},o,{email:e.target.value});})}}),
          React.createElement("button",{style:Object.assign({},S.btn,{width:"100%",marginBottom:16,fontSize:13}),onClick:saveOffice},"+ Add Office"),
          React.createElement("div",{style:{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:8}},"Current Offices"),
          loading?React.createElement("div",{style:{textAlign:"center",color:"#64748b",padding:"20px"}},"Loading..."):
          offices.map(function(o){
            return React.createElement("div",{key:o.code,style:{background:"#f8fafc",borderRadius:10,padding:"10px 12px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}},
              React.createElement("div",null,
                React.createElement("div",{style:{fontWeight:700,fontSize:13,color:"#0f172a"}},o.code+" — "+o.name),
                React.createElement("div",{style:{fontSize:11,color:"#64748b"}},o.email)
              ),
              React.createElement("button",{style:{background:"#fef2f2",color:"#dc2626",border:"1px solid #fecaca",borderRadius:8,padding:"4px 10px",fontSize:11,cursor:"pointer"},
                onClick:function(){deleteOffice(o.code);}
              },"Remove")
            );
          })
        ),
        ...
        // BREVO TAB
        tab==="brevo"&&React.createElement("div",null,
          React.createElement("div",{style:{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:10}},"Add Brevo API Key"),
          React.createElement("input",{style:Object.assign({},S.input,{marginBottom:8,fontSize:12}),placeholder:"Office Code (or 'default')",value:newBrevo.officeCode,onChange:function(e){setNewBrevo(function(b){return Object.assign({},b,{officeCode:e.target.value.toUpperCase()});})}}),
          React.createElement("input",{style:Object.assign({},S.input,{marginBottom:8,fontSize:12}),placeholder:"Label (e.g. PANDALAM KEY)",value:newBrevo.label,onChange:function(e){setNewBrevo(function(b){return Object.assign({},b,{label:e.target.value});})}}),
          React.createElement("input",{style:Object.assign({},S.input,{marginBottom:8,fontSize:11,fontFamily:"monospace"}),placeholder:"xkeysib-...",value:newBrevo.apiKey,onChange:function(e){setNewBrevo(function(b){return Object.assign({},b,{apiKey:e.target.value});})}}),
          React.createElement("button",{style:Object.assign({},S.btn,{width:"100%",marginBottom:16,fontSize:13}),onClick:saveBrevoKey},"+ Add Key"),
          React.createElement("div",{style:{fontWeight:700,fontSize:13,color:"#0f172a",marginBottom:8}},"Current Keys"),
          loading?React.createElement("div",{style:{textAlign:"center",color:"#64748b",padding:"20px"}},"Loading..."):
          brevoKeys.map(function(k){
            return React.createElement("div",{key:k.officeCode,style:{background:"#f8fafc",borderRadius:10,padding:"10px 12px",marginBottom:8}},
              React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}},
                React.createElement("div",{style:{fontWeight:700,fontSize:13,color:"#0f172a"}},k.officeCode==="default"?"🔑 Default Key":"🏢 "+k.officeCode+(k.label?" — "+k.label:"")),
                React.createElement("button",{style:{background:"#fef2f2",color:"#dc2626",border:"1px solid #fecaca",borderRadius:8,padding:"4px 10px",fontSize:11,cursor:"pointer"},
                  onClick:function(){deleteBrevoKey(k.officeCode);}
                },"Remove")
              ),
              React.createElement("div",{style:{fontSize:10,color:"#94a3b8",fontFamily:"monospace"}},k.apiKey.substring(0,30)+"...")
            );
          })
        ),
        ...
        tab==="stats"&&React.createElement("div",null,
          React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}},
            React.createElement("div",{style:{fontWeight:700,fontSize:13,color:"#0f172a"}},"Today's Reports"),
            React.createElement("button",{style:{background:"#f1f5f9",border:"none",borderRadius:8,padding:"6px 12px",fontSize:11,cursor:"pointer"},onClick:loadAll},"🔄 Refresh")
          ),
          loading?React.createElement("div",{style:{textAlign:"center",color:"#64748b",padding:"20px"}},"Loading..."):
          Object.keys(stats).length===0
            ?React.createElement("div",{style:{textAlign:"center",color:"#94a3b8",padding:"20px"}},"No reports today")
            :React.createElement("div",null,
                Object.keys(stats).map(function(code){
                  var s=stats[code];
                  return React.createElement("div",{key:code,style:{background:"#f0fdf4",borderRadius:10,padding:"12px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}},
                    React.createElement("div",null,
                      React.createElement("div",{style:{fontWeight:700,fontSize:13,color:"#0f172a"}},s.name||code),
                      React.createElement("div",{style:{fontSize:11,color:"#64748b"}},"Office: "+code)
                    ),
                    React.createElement("div",{style:{fontSize:24,fontWeight:800,color:"#15803d"}},s.count)
                  );
                }),
                React.createElement("div",{style:{background:"#0f172a",borderRadius:10,padding:"12px",marginTop:8,display:"flex",justifyContent:"space-between",alignItems:"center"}},
                  React.createElement("div",{style:{color:"#94a3b8",fontSize:13,fontWeight:700}},"Total Today"),
                  React.createElement("div",{style:{color:"#fff",fontSize:24,fontWeight:800}},
                    Object.values(stats).reduce(function(sum,s){return sum+(s.count||0);},0)
                  )
                )
              )
        )
...
function App(){
  ...
  function handleLogoTap(){
    ...
    setLogoTaps(newTaps);
    if(newTaps>=ADMIN_LOGO_TAPS){setLogoTaps(0);setShowAdmin(true);}
  ...
  useEffect(function(){
    var interval=setInterval(async function(){
      ...
      var minReport=todayQ[0];
      var fullReport=await dbGet(minReport.reportId);
      if(!fullReport){await removeFromQueue(minReport.reportId);refreshQueue();return;}
      try{
  ...
  async function handleValidateOffice(){
    ...
    setError("");
    var result=await backendGet("validateOffice",{code:officeCode.trim()});
    if(result.success){
      var officeData={
        code      :result.officeCode,
        name      :result.officeName,
        email     :result.officeEmail,
        brevoKey  :result.brevoApiKey,
        emailSender:result.emailSender||"brevo",
        gmailClientId:result.gmailClientId||"",
        gmailSecret  :result.gmailSecret||"",
        gmailToken   :result.gmailToken||"",
      };
  ...
  // Send report (Brevo or Gmail based on settings)
  async function sendReport(payload){
    if(payload.emailSender==="gmail"&&payload.gmailClientId&&payload.gmailToken){
      await sendViaGmail(
        payload.gmailClientId,payload.gmailSecret,payload.gmailToken,
        payload.to,payload.subject,payload.bodyText,payload.pdfBase64,payload.pdfFilename
      );
    } else {
      await sendViaBrevo(payload.brevoKey,payload.to,payload.subject,payload.bodyText,payload.pdfBase64,payload.pdfFilename);
    }
    // Log to sheets
    backendPost({action:"logReport",officeCode:payload.officeCode,officeName:payload.officeName});
  }
  ...
  async function handleSubmit(){
    ...
    try{
      ...
      var payload={
        ...
        emailSender :office.emailSender||"brevo",
        gmailClientId:office.gmailClientId||"",
        gmailSecret  :office.gmailSecret||"",
        gmailToken   :office.gmailToken||"",
    ...
    React.createElement("div",{style:S.body},
      ...
      React.createElement("div",{style:S.card},
        ...
        React.createElement("div",{style:{marginBottom:14}},
          ...
          React.createElement("input",{style:S.input,placeholder:"e.g. KL03AA0006",value:vehicle.reg,onChange:function(e){setVehicle(function(v){return Object.assign({},v,{reg:e.target.value.toUpperCase()});})}})
        ),
        React.createElement("div",{style:S.twoCol},
          React.createElement("div",{style:{flex:1}},
            React.createElement("label",{style:S.label},"Make & Model"),
            React.createElement("input",{style:S.input,placeholder:"e.g. Maruti Swift",value:vehicle.make,onChange:function(e){setVehicle(function(v){return Object.assign({},v,{make:e.target.value});})}})
          ),
          React.createElement("div",{style:{flex:1}},
            React.createElement("label",{style:S.label},"Owner Name"),
            React.createElement("input",{style:S.input,placeholder:"Full name",value:vehicle.owner,onChange:function(e){setVehicle(function(v){return Object.assign({},v,{owner:e.target.value});})}})
          )
        ),
        React.createElement("div",null,
          React.createElement("label",{style:S.label},"Expired Policy Number"),
          React.createElement("input",{style:S.input,placeholder:"e.g. POL/2023/00123",value:vehicle.policy,onChange:function(e){setVehicle(function(v){return Object.assign({},v,{policy:e.target.value});})}})
        )
      ),
